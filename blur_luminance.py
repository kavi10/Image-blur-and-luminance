...
def set_luminance(fptr, l_val):
    ...
    **Returns**
    ...
    for x in range(width):
        for y in range(height):           
            pxl_new = imgpxls[y,x] + (delta // 3)
            img.putpixel((x, y), (int(pxl_new[0]), int(pxl_new[1]), int(pxl_new[2])))
...
def get_luminance(img):
    ...
    **Returns**
    ...
    for x in range(width):
        for y in range(height):
            l_pxl.append(get_pxl_luminance(imgpxls[y,x]))
...
if __name__ == "__main__":
    fptr = "spring.png" 
    # fptrlum = "spring_lum.png"
    blur(fptr)
    set_luminance(fptr, 255.0)
    # print(get_luminance(Image.open(fptr).convert("RGB")), get_luminance(Image.open(fptrlum).convert("RGB")))
